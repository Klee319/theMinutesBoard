...
- JSON形式でのエクスポート
- CSV形式でのエクスポート
- ファイル名の自動生成（会議タイトル_日付形式）

### 📚 会議履歴管理
- 日付範囲での会議履歴フィルタリング
- キーワード検索機能
- 会議一覧の表示（新しい順にソート）
- タイトルなし会議の自動タイトル生成（「会議 YYYY/MM/DD HH:MM」形式）
- ページネーション（10件ごと）
- 議事録の有無でフィルタリング
...
- **AI統合**: 複数のAIプロバイダーAPI（レート制限対応）
- **状態管理**: Zustand
- **データベース**: IndexedDB（オプション）、Chrome Storage
- **コード品質**: 
  - 共通定数の集約化（constants/config.ts）
  - 関数のモジュール化
  - 設定の外部化
  - ロギングシステム（logger）
  - パフォーマンス監視（performance-monitor）
...
### v2.2.2 (2025年7月17日)
- 🔧 AIサービスのレート制限値を設定ファイルに外部化
- 🔧 ストレージ制限値を設定ファイルに外部化  
- 🔧 AIプロンプトサイズ制限を設定可能に
- 🐛 すべてのconsole文をloggerに統一化
- 📝 定数定義の整理と集約化（config.ts）
- ♻️ ハードコードされた値の完全な除去
